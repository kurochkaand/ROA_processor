...
Here is how 2nd and the last rows look like:
"""
...
The wavenumber axis should be reversed to increasing order (from low to high).
For spike replacement: replace bad point by median of clean blocks at same wavenumber.
For Raman baseline subtraction, I normally record spectrum of water (solvent) in the same cuvette at same laser power, I record empty cuvette at same laser power and I record empty sample space to get air spectrum. empty cuvette - air spectrum gives me glass spectrum. water in cuvette - glass gives me pure water Raman. Often I measure not transparent samples that have higher content of glass in the Raman spectra. 
I can provide path to reference spectra, but it should be optional.
Do this:
first build a minimal but solid version that does file discovery → parsing → cumulative-to-isolated blocks → spike removal → averaging → plotting/export.



Please
1. add a parameter to a function call something like --min-wavenumber 200 
which will cut out low wavenumber before 200 1/cm.
2. "--output processed" should save results into "processed" directory inside the same directory where are the data files not where the program is located.
3. I am confident that in the range 1800 - 2609.36 1/cm the ROA spectrum should appear as a strait horizontal line, instead it is quite noicy, can you think of some algorithm that whould denoice the whole spectrum by using the 1800 - 2609.36 1/cm range as a quality control.